// C++ Program to Implement Circular Doubly Linked List
    ...
    {
	int info;
	struct node *next;		 // Declaration   Node   
	struct node *prev;
...

    {
...

        {
            ...
            cout<<"Variations on Doubly Circular linked list"<<endl;
            
            cout<<"1.Insert at Beginning"<<endl;
            cout<<"2.Insert at Last"<<endl;			//operations
...

    {
        counter++;  
        struct node *temp;
        temp = new(struct node);    				//Allocating memory for Node
        temp->info = value;
        temp->next = NULL;
        temp->prev = NULL;
        return temp;
...

    {
        ...
        cin>>value;						// Insert node at starting.
        struct node *temp;
    ...
    {
        int value;    
        cout<<endl<<"Enter the element to be inserted: ";			// Insert node at last
        cin>>value; 
        struct node *temp;
        temp = create_node(value);
    ...
    {    
        int value, pos, i;
        cout<<endl<<"Enter the element to be inserted: ";
        cin>>value;							// Insert node at given position
    ...
    {    
        ...
        if (start == last && start == NULL)
        {
            ...
            return;						//  Delete Node from given Position
        }
...

    {
        ...
        if (start == last && start == NULL)
        {
            cout<<"The List is empty, nothing to update"<<endl;			 //Update value of a given node 
            return; 
        }
...

    {
        ...
        bool flag = false;
        struct node *s;
        if (start == last && start == NULL)				//Search Element in the list
        {
...

    {
        ...
        if (start == last && start == NULL)
        {
            cout<<"The List is empty, nothing to sort"<<endl;	//  Sorting Doubly Circular Link List
            return;
        }
        s = start;
        for (i = 0;i < counter;i++)
...

    {
        int i;
        struct node *s;
        if (start == last && start == NULL)			// Display Elements of the List 
        {
    ...
    {
        if (start == last && start == NULL)				// Reverse Doubly Circular Linked List 
        {
